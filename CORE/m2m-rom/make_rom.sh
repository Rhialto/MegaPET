...
##############################################################################
# PET specific: Generate osm_const.asm
##############################################################################
...

{
    ...
    echo "; ../vhdl/globals.vhd and in ../vhdl/config.vhd"
    echo ""
    echo "; -----------------------------------------------------------------"
    echo "; PET specific OSM item constants"
    echo "; -----------------------------------------------------------------"
    echo ""
} > osm_const.asm

awk '/constant C_MENU_/ {gsub("C_MENU_", "PET_OSM_"); gsub(";", ""); printf("%-27s .EQU %s\n", $2, $6)}' ../vhdl/globals.vhd >> osm_const.asm

{
    echo ""
    echo "; -----------------------------------------------------------------"
    echo "; PET specific menu group items"
    echo "; -----------------------------------------------------------------"
    echo ""
} >> osm_const.asm

awk '/constant OPTM_G_/ && !/16#/ {gsub("OPTM_G_", "PET_OPTM_G_"); gsub(";", ""); printf("%-27s .EQU %s\n", $2, $6)}' ../vhdl/config.vhd >> osm_const.asm
